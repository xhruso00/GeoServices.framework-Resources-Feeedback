...
- (void)_readContext {
    ...
    @synchronized(reader) {
        if (!_flags.read_context) {
            ...
            GEORPSearchFeedbackReadSpecified(self, reader, tags);
        }
    }
}
- (BOOL)hasContext
...
- (void)setContext:(GEORPSearchFeedbackContext *)v
{
...
- (void)setCorrections:(GEORPSearchFeedbackCorrections *)v
{
...
- (NSDictionary *)dictionaryRepresentation
{
    ...
    {
        GEORPSearchFeedbackContext * value = self.context;
        if (value) {
    ...
    {
        GEORPSearchFeedbackCorrections * value = self.corrections;
        if (value) {
...
- (void)readAll:(BOOL)recursively {
    ...
    @synchronized(reader) {
        ...
        GEORPSearchFeedbackReadSpecified(self, reader, recursively ? recursiveTag : nonRecursiveTag);
    }
}
BOOL GEORPSearchFeedbackReadAllFrom(__unsafe_unretained GEORPSearchFeedback *self, __unsafe_unretained PBDataReader *reader, BOOL recursively) {
    if (self->_readerMarkLength != 0) {
        [NSException raise:NSInvalidArgumentException format:@"GEORPSearchFeedbackReadAllFrom can only be called once per object"];
        return NO;
    }
    ...
    @synchronized(reader) {
        ...
        BOOL result = GEORPSearchFeedbackReadSpecified(self, reader, recursively ? recursiveTag : initialTag);
        return result;
    }
}

BOOL GEORPSearchFeedbackReadFrom(__unsafe_unretained GEORPSearchFeedback *self, __unsafe_unretained PBDataReader *reader) {
    @synchronized(reader) {
        return GEORPSearchFeedbackReadAllFrom(self, reader, NO);
    }
}

BOOL GEORPSearchFeedbackReadSpecified(__unsafe_unretained GEORPSearchFeedback *self, __unsafe_unretained PBDataReader *reader, const uint32_t *tags) {
    if (reader == nil) {
        return NO;
    }
    ...
    while (PBReaderHasMoreData(reader)) {
        ...
        switch (tag) {
            ...
            case 1 /* context */:
            {
                ...
                GEORPSearchFeedbackContext *new_context = [[GEORPSearchFeedbackContext alloc] init];
                ...
                BOOL inError = !GEORPSearchFeedbackContextReadAllFrom(new_context, reader, recursively);
                if (inError)
                {
                    return NO;
                }
            ...
            case 2 /* corrections */:
            {
                ...
                GEORPSearchFeedbackCorrections *new_corrections = [[GEORPSearchFeedbackCorrections alloc] init];
                ...
                BOOL inError = !GEORPSearchFeedbackCorrectionsReadAllFrom(new_corrections, reader, recursively);
                if (inError)
                {
                    return NO;
                }
...
{
    return GEORPSearchFeedbackIsValid(reader);
}

BOOL GEORPSearchFeedbackIsValid(__unsafe_unretained PBDataReader *reader) {
            ...
            case 1 /* context */:
                    ...
                    returnValue(NO);
                {
                    ...
                    if (!PBReaderPlaceMark(reader, &mark_context))
                        returnValue(NO);
                    if (!GEORPSearchFeedbackContextIsValid(reader))
                        returnValue(NO);
                    PBReaderRecallMark(reader, &mark_context);
                }
                break;
            case 2 /* corrections */:
                    ...
                    returnValue(NO);
                {
                    ...
                    if (!PBReaderPlaceMark(reader, &mark_corrections))
                        returnValue(NO);
                    if (!GEORPSearchFeedbackCorrectionsIsValid(reader))
                        returnValue(NO);
                    PBReaderRecallMark(reader, &mark_corrections);
                }
                break;
...
- (void)writeTo:(PBDataWriter *)writer
{
    ...
    @synchronized(reader) {
        if (reader) {
            if (!GEORPSearchFeedbackIsDirty(self)) {
...
BOOL GEORPSearchFeedbackIsDirty(__unsafe_unretained GEORPSearchFeedback *self)
{
    if (self == nil) {
        return NO;
    }
    ...
    BOOL dirty = NO
        ...
        || (self->_flags.read_context && GEORPSearchFeedbackContextIsDirty(self->_context))
...
- (void)copyTo:(GEORPSearchFeedback *)other
{
    [self readAll:NO];
    os_unfair_lock_lock_with_options(&_readerLock, OS_UNFAIR_LOCK_DATA_SYNCHRONIZATION);
...
{
    GEORPSearchFeedback *copy = [[[self class] allocWithZone:zone] init];
    os_unfair_lock_lock_with_options(&_readerLock, OS_UNFAIR_LOCK_DATA_SYNCHRONIZATION);
    PBDataReader *reader = _reader;
    os_unfair_lock_unlock(&_readerLock);
    @synchronized(reader) {
        if (reader) {
            ...
            if (!wroteAny) {
                reader->_length = (NSUInteger)_readerMarkLength;
                [reader seekToOffset:(NSUInteger)_readerMarkPos];
                GEORPSearchFeedbackReadAllFrom(copy, reader, NO);
                return copy;
            }
            [self readAll:NO];
        }
...
{
    GEORPSearchFeedback *other = (GEORPSearchFeedback *)object;
...
- (void)mergeFrom:(GEORPSearchFeedback *)other
{
    [other readAll:NO];
...
- (void)clearUnknownFields:(BOOL)recursively
{
    ...
    [self readAll:NO];
    [_context clearUnknownFields:YES];
