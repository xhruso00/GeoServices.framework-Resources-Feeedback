...
- (void)setAddress:(GEORPAddress *)v
{
    _address = v;
}
...
- (NSDictionary *)dictionaryRepresentation
{
    ...
    {
        GEORPAddress * value = self.address;
        if (value) {
...
BOOL GEORPPersonalizedMapsReadAllFrom(__unsafe_unretained GEORPPersonalizedMaps *self, __unsafe_unretained PBDataReader *reader, BOOL recursively) {
    static const uint32_t recursiveTag[] = {TAG_PARSE_ALL_RECURSIVE, 0};

    @synchronized(reader) {
        BOOL result = GEORPPersonalizedMapsReadSpecified(self, reader, recursively ? recursiveTag : nil);
        return result;
    }
}
BOOL GEORPPersonalizedMapsReadFrom(__unsafe_unretained GEORPPersonalizedMaps *self, __unsafe_unretained PBDataReader *reader) {
    @synchronized(reader) {
        return GEORPPersonalizedMapsReadAllFrom(self, reader, NO);
    }
}

BOOL GEORPPersonalizedMapsReadSpecified(__unsafe_unretained GEORPPersonalizedMaps *self, __unsafe_unretained PBDataReader *reader, const uint32_t *tags) {
    if (reader == nil) {
        return NO;
    }
    ...
    #define shouldParseTag YES
        ...
        switch (tag) {
            ...
            case 3 /* address */:
            {
                GEORPAddress *new_address = [[GEORPAddress alloc] init];
                ...
                BOOL inError = !GEORPAddressReadAllFrom(new_address, reader, recursively);
                if (inError)
                {
                    return NO;
                }
...
{
    return GEORPPersonalizedMapsIsValid(reader);
}

BOOL GEORPPersonalizedMapsIsValid(__unsafe_unretained PBDataReader *reader) {
...
- (void)copyTo:(GEORPPersonalizedMaps *)other
{
    [self readAll:NO];
    if (self->_flags.has_addressType)
    {
...
{
    GEORPPersonalizedMaps *copy = [[[self class] allocWithZone:zone] init];
    if (self->_flags.has_addressType)
    {
...
{
    GEORPPersonalizedMaps *other = (GEORPPersonalizedMaps *)object;
...
- (void)mergeFrom:(GEORPPersonalizedMaps *)other
{
    [other readAll:NO];
